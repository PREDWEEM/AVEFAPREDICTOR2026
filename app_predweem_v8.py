...
# Adaptado especialmente para archivos XLSX con hojas:
# Fecha, JD, Temperatura_Minima, Temperatura_Maxima,
# Precipitacion_Pluviometrica — Bordenave 1977–2015
# ===============================================================
...
# ===============================================================
# ANN — Carga de pesos y predicción
# ===============================================================
class ANN:
    def __init__(self):
...
# ===============================================================
# LECTOR XLSX MULTI-AÑO — Bordenave_1977_2015
# ===============================================================
...
# Si es CSV, solo un año
if uploaded.name.lower().endswith(".csv"):
    df_raw = pd.read_csv(uploaded)
    df_raw.columns = [c.lower() for c in df_raw.columns]
    mapping = {
        "jd":"JD","tmin":"TMIN","tmax":"TMAX","prec":"Prec",
...
fig.add_trace(go.Scatter(x=df_meteo["JD"], y=df_meteo["MA5"], mode="lines", line=dict(color="black",width=3)))
fig.update_layout(yaxis_title="EMERREL (0–1)", xaxis_title="JD")
st.plotly_chart(fig, use_container_width=True)

st.subheader("📈 EMEAC (ANN)")
fig2 = go.Figure()
fig2.add_trace(go.Scatter(x=df_meteo["JD"], y=df_meteo["EMEAC"], mode="lines", line=dict(color="green",width=3)))
...
if len(años)>1:
    ...
    if st.button("Ejecutar ANN + clasificación para todos los años"):
        ...
        for a,dfy in años.items():
            emer,emac = ann.predict_emerrel(dfy)
            JDy = dfy["JD"].to_numpy()
            ...
            registros.append({
                ...
                "JD25": calc_percentile_day(JDy,emac,0.25),
                "JD50": calc_percentile_day(JDy,emac,0.50),
                "JD75": calc_percentile_day(JDy,emac,0.75),
                "JD95": calc_percentile_day(JDy,emac,0.95)
            })
